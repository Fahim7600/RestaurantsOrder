...
## Implemented additional feature: Group Order Session (Shared Table Cart)

### The Chosen Feature
A real-time shared ordering session for dine-in tables and group takeaway orders, built directly into the UI (Cart Drawer & Checkout). A host diner taps "Group Order" in the cart drawer to create a lightweight room session (generating a 4-digit code like `ROOM-4821`). Table companions can join using the code or be added dynamically on the fly (`+ Add Diner`). As each companion adds items to the cart, the system tags the item with their name and computes a live, itemized per-person bill breakdown at checkout.
...
3. **Instant Itemized Bill Splitting**: Each dish in the shared cart is tagged with the custom name of the diner who added it. At checkout, the app displays both a grand total and an exact per-person breakdown, eliminating awkward mental math and manual bill calculation after the meal.
